...
and they are about your **PHP**, not about this plugin's native code.

**Native code refreshes on its own from 4.0.0.** Earlier versions kept a stale
copy of a plugin's Swift/Kotlin in the generated project, so editing the plugin
and re-running built the OLD code and you had to copy it across by hand. 4.0.0
cleans the generated copy before compiling. If you are upgrading, run
`php artisan native:install both --force` once — the fix only takes effect on
a project regenerated by 4.x.

...
- **The 4.0.0 workaround is documented as a 4.0.0 workaround.** NativePHP
  4.0.1 recreates the runtime directories its bundle copy excludes, so the
  `config/view.php` this README told every user to publish is no longer
  needed. Kept for anyone pinned to 4.0.0 exactly, and clearly marked as such.
  Verified by building both platforms on 4.0.1 with the published config
  removed.
...
- **The package no longer carries screenshots.** They were 2.8 MB of a 3.8 MB
  download — nearly three quarters of it — for files no application ever reads.
  A plugin should ship the plugin. The README points at the demo application
  instead, which is where seeing it running belongs.

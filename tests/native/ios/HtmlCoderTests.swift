...
// ── Checklists ──────────────────────────────────────────────────────────────
print("")
print("Checklists — a list whose items carry a state")
do {
    let html = "<ul data-checklist><li data-checked=\"true\">Ship it</li>"
    ...
    same("it is a <ul>, so an unaware renderer still sees a list",
         String(HtmlCoder.emit(HtmlCoder.parse(html)).html.prefix(22)), "<ul data-checklist><li")
    ...
    same("and back out of it", HtmlCoder.emit(JsonCoder.decode(JsonCoder.encode(blocks))).html, html)
